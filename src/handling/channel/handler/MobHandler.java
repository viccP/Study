...
public class MobHandler
{
  public static final void MoveMonster(SeekableLittleEndianAccessor slea, MapleClient c, MapleCharacter chr)
  {
    ...
    if (useSkill)
    {
      ...
      if (size > 0)
      {
        Pair<Integer, Integer> skillToUse = monster.getSkills().get(Randomizer.nextInt(size));
  ...
  public static final void AutoAggro(int monsteroid, MapleCharacter chr)
  {
    ...
    if ((monster != null) && (chr.getPosition().distanceSq(monster.getPosition()) < 200000.0D)) {
  ...
  {
    chr.getMap().getMonsterByOid(slea.readInt());
  }
