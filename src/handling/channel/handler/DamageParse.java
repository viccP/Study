...
public class DamageParse {
	private static final long DAMAGE_UPPER_LIMIT_LONG = 199999999999L;
	private static final double DAMAGE_UPPER_LIMIT_DOUBLE = 199999999999.0;
	...
	public static void applyAttack(AttackInfo attack, ISkill theSkill, MapleCharacter player, int attackCount, double maxDamagePerMonster, MapleStatEffect effect, AttackType attack_type) {
		...
		for (AttackPair oned : attack.allDamage) {
				...
				if (fixeddmg != -1) {
					...
					if (Tempest) {
						...
						if (eachd.intValue() > maxDamagePerHit) {
							player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE);
							if (eachd.intValue() > maxDamagePerHit * 2.0) {
			...
			if (attack.skill != 1221011) {
				...
				monster.damage(player, monster.getStats().isBoss() ? DAMAGE_UPPER_LIMIT_LONG : monster.getHp() - 1L, true, attack.skill);
			}
			if (monster.isBuffed(MonsterStatus.WEAPON_DAMAGE_REFLECT)) {
				player.addHP(-(7000 + Randomizer.nextInt((int) 8000)));
			}
			if (stats.hpRecoverProp > 0 && Randomizer.nextInt((int) 100) <= stats.hpRecoverProp) {
				player.healHP(stats.hpRecover);
			}
			if (stats.mpRecoverProp > 0 && Randomizer.nextInt((int) 100) <= stats.mpRecoverProp) {
			...
			case 4101005:
			case 5111004: {
				stats.setHp(stats.getHp() + (int) Math.min(monster.getMobMaxHp(), (long) Math.min((int) ((double) totDamage * (double) theSkill.getEffect((int) player.getSkillLevel(theSkill)).getX() / 100.0), stats.getMaxHp() / 2)), true);
				break;
			}
			case 5211006:
			case 5220011:
			...
			case 4341005: {
				...
				int i;
				ISkill skill = SkillFactory.getSkill((int) 4120005);
				ISkill skill2 = SkillFactory.getSkill((int) 4220005);
				ISkill skill3 = SkillFactory.getSkill((int) 4340001);
				if (player.getSkillLevel(skill) > 0) {
					venomEffect = skill.getEffect((int) player.getSkillLevel(skill));
					for (i = 0; i < attackCount; ++i) {
						if (!venomEffect.makeChanceResult() || monster.getVenomMulti() >= 3)
							continue;
						monster.setVenomMulti((byte) (monster.getVenomMulti() + 1));
						MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.POISON, Integer.valueOf(1), 4120005, null, false);
						monster.applyStatus(player, monsterStatusEffect, false, (long) venomEffect.getDuration(), true);
					}
				} else if (player.getSkillLevel(skill2) > 0) {
					venomEffect = skill2.getEffect((int) player.getSkillLevel(skill2));
					for (i = 0; i < attackCount; ++i) {
						if (!venomEffect.makeChanceResult() || monster.getVenomMulti() >= 3)
							continue;
						monster.setVenomMulti((byte) (monster.getVenomMulti() + 1));
						MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.POISON, Integer.valueOf(1), 4220005, null, false);
						monster.applyStatus(player, monsterStatusEffect, false, (long) venomEffect.getDuration(), true);
					}
				} else {
					if (player.getSkillLevel(skill3) <= 0)
						break;
					venomEffect = skill3.getEffect((int) player.getSkillLevel(skill3));
					for (i = 0; i < attackCount; ++i) {
						if (!venomEffect.makeChanceResult() || monster.getVenomMulti() >= 3)
							continue;
						monster.setVenomMulti((byte) (monster.getVenomMulti() + 1));
						MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.POISON, Integer.valueOf(1), 4340001, null, false);
						monster.applyStatus(player, monsterStatusEffect, false, (long) venomEffect.getDuration(), true);
					}
				}
				break;
			...
			case 21120010: {
				...
				if (player.getBuffedValue(MapleBuffStat.WK_CHARGE) != null && !monster.getStats().isBoss() && (eff = player.getStatForBuff(MapleBuffStat.WK_CHARGE)) != null && eff.getSourceId() == 21111005) {
					monster.applyStatus(player, new MonsterStatusEffect(MonsterStatus.SPEED, Integer.valueOf(eff.getX()), eff.getSourceId(), null, false), false, (long) (eff.getY() * 1000), false);
				}
				if (player.getBuffedValue(MapleBuffStat.BODY_PRESSURE) == null || monster.getStats().isBoss() || (eff = player.getStatForBuff(MapleBuffStat.BODY_PRESSURE)) == null || !eff.makeChanceResult() || monster.isBuffed(MonsterStatus.NEUTRALISE))
					break;
				monster.applyStatus(player, new MonsterStatusEffect(MonsterStatus.NEUTRALISE, Integer.valueOf(1), eff.getSourceId(), null, false), false, (long) (eff.getX() * 1000), false);
				break;
			}
			}
			if (totDamageToOneMonster > 0) {
				...
				if (weapon_ != null && (stat = GameConstants.getStatFromWeapon((int) weapon_.getItemId())) != null && Randomizer.nextInt((int) 100) < GameConstants.getStatChance()) {
					monsterStatusEffect = new MonsterStatusEffect(stat, Integer.valueOf(GameConstants.getXForStat((MonsterStatus) stat)), GameConstants.getSkillForStat((MonsterStatus) stat), null, false);
				...
				if (player.getBuffedValue(MapleBuffStat.BLIND) != null && (eff = player.getStatForBuff(MapleBuffStat.BLIND)).makeChanceResult()) {
					monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.ACC, Integer.valueOf(eff.getX()), eff.getSourceId(), null, false);
					monster.applyStatus(player, monsterStatusEffect, false, (long) (eff.getY() * 1000), false);
				}
				if (player.getBuffedValue(MapleBuffStat.HAMSTRING) != null && (eff2 = (skill = SkillFactory.getSkill((int) 3121007)).getEffect((int) player.getSkillLevel(skill))).makeChanceResult()) {
					MonsterStatusEffect monsterStatusEffect2 = new MonsterStatusEffect(MonsterStatus.SPEED, Integer.valueOf(eff2.getX()), 3121007, null, false);
					monster.applyStatus(player, monsterStatusEffect2, false, (long) (eff2.getY() * 1000), false);
				}
				if (player.getJob() == 121) {
					for (int charge : charges) {
						ISkill skill2 = SkillFactory.getSkill((int) charge);
						if (!player.isBuffFrom(MapleBuffStat.WK_CHARGE, skill2))
							continue;
						MonsterStatusEffect monsterStatusEffect3 = new MonsterStatusEffect(MonsterStatus.FREEZE, Integer.valueOf(1), charge, null, false);
						monster.applyStatus(player, monsterStatusEffect3, false, (long) (skill2.getEffect((int) player.getSkillLevel(skill2)).getY() * 2000), false);
						break;
					}
				}
			}
			...
			for (Map.Entry<MonsterStatus, Integer> z : effect.getMonsterStati().entrySet()) {
				monster.applyStatus(player, new MonsterStatusEffect((MonsterStatus) z.getKey(), (Integer) z.getValue(), theSkill.getId(), null, false), effect.isPoison(), (long) effect.getDuration(), false);
			}
		}
	...
	public static final void applyAttackMagic(AttackInfo attack, ISkill theSkill, MapleCharacter player, MapleStatEffect effect) {
		...
		if (GameConstants.isMulungSkill((int) attack.skill)) {
			if (player.getMapId() / 10000 != 92502) {
				return;
			}
			player.mulung_EnergyModify(false);
		}
		if (GameConstants.isPyramidSkill((int) attack.skill)) {
			if (player.getMapId() / 1000000 != 926) {
				return;
			}
			if (player.getPyramidSubway() == null || !player.getPyramidSubway().onSkillUse(player)) {
		...
		} else if (GameConstants.isPyramidSkill((int) attack.skill)) {
			maxDamagePerHit = 1.0;
		} else {
		...
		int totDamage = 0;
		short CriticalDamage = stats.passive_sharpeye_percent();
		ISkill eaterSkill = SkillFactory.getSkill((int) GameConstants.getMPEaterForJob((int) player.getJob()));
		...
		for (AttackPair oned : attack.allDamage) {
			...
			if (eaterLevel <= 0)
				continue;
			eaterSkill.getEffect((int) eaterLevel).applyPassive(player, (MapleMapObject) monster);
		}
		if (attack.skill != 2301002) {
	...
	private static double CalculateMaxMagicDamagePerHit(MapleCharacter chr, ISkill skill, MapleMonster monster, MapleMonsterStats mobstats, PlayerStats stats, Element elem, Integer sharpEye, double maxDamagePerMonster) {
		...
		if (MinAccuracy > Accuracy && skill.getId() != 1000 && skill.getId() != 10001000 && skill.getId() != 20001000 && skill.getId() != 20011000 && skill.getId() != 30001000 && !GameConstants.isPyramidSkill((int) skill.getId())) {
			return 0.0;
		}
		...
		if (elemMaxDamagePerMob > DAMAGE_UPPER_LIMIT_DOUBLE) {
			elemMaxDamagePerMob = DAMAGE_UPPER_LIMIT_DOUBLE;
	...
	private static void handlePickPocket(final MapleCharacter player, final MapleMonster mob, AttackPair oned) {
		...
		ISkill skill = SkillFactory.getSkill((int) 4211003);
		MapleStatEffect s = skill.getEffect((int) player.getSkillLevel(skill));
		for (Pair<Integer, Boolean> eachde : oned.attack) {
			...
			Timer.MapTimer.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					player.getMap().spawnMesoDrop(Math.min((int) Math.max((double) eachd.intValue() / 20000.0 * (double) maxmeso, 1.0), maxmeso), new Point((int) (mob.getPosition().getX() + (double) Randomizer.nextInt((int) 100) - 50.0), (int) mob.getPosition().getY()), (MapleMapObject) mob, player, true, (byte) 0);
				}
			}, 100L);
		}
	}
	...
	private static double CalculateMaxWeaponDamagePerHit(MapleCharacter player, MapleMonster monster, AttackInfo attack, ISkill theSkill, MapleStatEffect attackEffect, double maximumDamageToMonster, Integer CriticalDamagePercent) {
		...
		if (player.getMapId() / 1000000 == 914) {
			return DAMAGE_UPPER_LIMIT_DOUBLE;
		}
		...
		if (theSkill != null) {
			...
			case 4331003: {
				maximumDamageToMonster = monster.getStats().isBoss() ? DAMAGE_UPPER_LIMIT_LONG : monster.getHp();
				defined = true;
				break;
			}
			case 3221007: {
				maximumDamageToMonster = monster.getStats().isBoss() ? DAMAGE_UPPER_LIMIT_LONG : monster.getMobMaxHp();
				defined = true;
				break;
			}
			case 1221011: {
				maximumDamageToMonster = monster.getStats().isBoss() ? DAMAGE_UPPER_LIMIT_LONG : monster.getHp() - 1L;
				defined = true;
				break;
			}
			case 4211006: {
		...
		if ((elementalMaxDamagePerMonster += elementalMaxDamagePerMonster * (monster.getStats().isBoss() ? stat.bossdam_r : stat.dam_r) / 100.0) > DAMAGE_UPPER_LIMIT_DOUBLE) {
			if (!defined) {
				elementalMaxDamagePerMonster = DAMAGE_UPPER_LIMIT_DOUBLE;
			}
	...
	public static final AttackInfo DivideAttack(AttackInfo attack, int rate) {
		...
		for (AttackPair p : attack.allDamage) {
			...
			while (i$.hasNext()) {
				Pair<Integer, Boolean> pair = i$.next();
	...
	public static final AttackInfo Modify_AttackCrit(AttackInfo attack, MapleCharacter chr, int type) {
		final byte CriticalRate = chr.getStat().passive_sharpeye_rate();
		...
		if (attack.skill != 4211006 && attack.skill != 3211003 && attack.skill != 4111004 && (CriticalRate > 0 || attack.skill == 4221001 || attack.skill == 3221007)) {
			for (AttackPair p : attack.allDamage) {
				if (p.attack == null)
					continue;
				int hit = 0;
				int mid_att = p.attack.size() / 2;
				List<Pair<Integer, Boolean>> eachd_copy = new ArrayList<Pair<Integer, Boolean>>(p.attack);
	...
	public static final AttackInfo parseDmgMa(LittleEndianAccessor lea, MapleCharacter chr) {
		...
		for (int i = 0; i < ret.targets; ++i) {
			...
			chr.getMap().getMonsterByOid(oid);
	...
	public static void Damage_Mob_Level(MapleCharacter c, MapleMonster monster, AttackInfo ret) {
		try {
			if (c.getLevel() < monster.getStats().getLevel() - 20 && ret.skill != 4211006 && c.getJob() > 422 && c.getJob() < 400) {
				...
				FileoutputUtil.packetLog((String) ("log\\越级打怪检测\\" + c.getName() + ".log"), (String) 越级打怪检测);
	...
	public static void Damage_Position(MapleCharacter c, MapleMonster monster, AttackInfo ret) {
		try {
			if (!GameConstants.不检测技能((int) ret.skill)) {
				if (c.getJob() >= 1300 && c.getJob() <= 1311 || c.getJob() >= 1400 && c.getJob() <= 1411 || c.getJob() >= 400 && c.getJob() <= 422 || c.getJob() >= 300 && c.getJob() <= 322 || c.getJob() == 500 || c.getJob() >= 520 && c.getJob() <= 522) {
					if (c.getPosition().y - monster.getPosition().y >= 800) {
						...
						FileoutputUtil.packetLog((String) ("log\\全屏检测\\" + c.getName() + ".log"), (String) 全屏);
	...
	public static final int Damage_SkillPD(MapleCharacter c, int damage, AttackInfo ret) {
		if (GameConstants.Novice_Skill((int) ret.skill)) {
			if (damage > 40) {
			...
			if (GameConstants.Ares_Skill_350((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 13)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Ares_Skill_140((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 20)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Ares_Skill_1500((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 21)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Ares_Skill_800((int) ret.skill) && c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 14)) {
				damage = 1;
				return damage;
			}
			...
			if (GameConstants.Warrior_Skill_450((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 11)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Warrior_Skill_550((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 18)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Warrior_Skill_900((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 12)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Warrior_Skill_2000((int) ret.skill) && c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 24)) {
				damage = 1;
				return damage;
			}
			...
			if (GameConstants.Magician_Skill_90((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 15)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Magician_Skill_180((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 18)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Magician_Skill_240((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 20)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Magician_Skill_670((int) ret.skill) && c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 36)) {
				damage = 1;
				return damage;
			}
		} else if (c.getJob() == 300 || c.getJob() == 310 || c.getJob() == 311 || c.getJob() == 312 || c.getJob() == 320 || c.getJob() == 321 || c.getJob() == 322) {
			if (GameConstants.Bowman_Skill_180((int) ret.skill) && c.getBuffedValue(MapleBuffStat.SHARP_EYES) != null && damage > 0) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 13)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Bowman_Skill_260((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 9)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Bowman_Skill_850((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 12)) {
					damage = 1;
					return damage;
				}
			...
			if (GameConstants.Bowman_Skill_180((int) ret.skill) && damage > 0) {
			...
			} else if (GameConstants.Bowman_Skill_850((int) ret.skill) && c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 8)) {
				damage = 1;
				return damage;
			}
		} else if (c.getJob() == 400 || c.getJob() == 410 || c.getJob() == 411 || c.getJob() == 412 || c.getJob() == 420 || c.getJob() == 421 || c.getJob() == 422) {
			if (GameConstants.Thief_Skill_180((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 11)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Thief_Skill_250((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 14)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Thief_Skill_500((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 18)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Thief_Skill_800((int) ret.skill) && c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 25)) {
				damage = 1;
				return damage;
			}
		} else if (c.getJob() == 500 || c.getJob() == 510 || c.getJob() == 511 || c.getJob() == 512 || c.getJob() == 520 || c.getJob() == 521 || c.getJob() == 522) {
			if (GameConstants.Pirate_Skill_290((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 8)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Pirate_Skill_420((int) ret.skill)) {
				if ((double) c.getStat().getCurrentMaxBaseDamage() <= (double) damage / 9.3) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Pirate_Skill_700((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 13)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Pirate_Skill_810((int) ret.skill)) {
				if ((double) c.getStat().getCurrentMaxBaseDamage() <= (double) damage / 13.2) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Pirate_Skill_1200((int) ret.skill) && c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 18)) {
				damage = 1;
				return damage;
			}
		} else if (c.getJob() == 1000 || c.getJob() == 1100 || c.getJob() == 1110 || c.getJob() == 1111) {
			if (GameConstants.Ghost_Knight_Skill_320((int) ret.skill)) {
			...
			if (GameConstants.Fire_Knight_Skill_140((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 13)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Fire_Knight_Skill_500((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 8)) {
					damage = 1;
					return damage;
				}
			...
			if (GameConstants.Wind_Knight_Skill_160((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 8)) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Wind_Knight_Skill_550((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 11)) {
					damage = 1;
					return damage;
				}
			...
			if (GameConstants.Night_Knight_Skill_220((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 9)) {
					damage = 1;
					return damage;
				}
			...
			if (GameConstants.Thief_Skill_270((int) ret.skill)) {
				if ((double) c.getStat().getCurrentMaxBaseDamage() <= (double) damage / 7.7) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Thief_Skill_420((int) ret.skill)) {
				if ((double) c.getStat().getCurrentMaxBaseDamage() <= (double) damage / 10.2) {
					damage = 1;
					return damage;
				}
			} else if (GameConstants.Thief_Skill_650((int) ret.skill)) {
				if (c.getStat().getCurrentMaxBaseDamage() <= (float) (damage / 14)) {
					damage = 1;
					return damage;
				}
