...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("你好,我是玩具城人见人爱的玩具城店长!如果你有#b玩具城美发店高级会员卡#k或#b玩具城染发高级会员卡#k,你就放心的把发型交给我,我会让你满意的.那么你要做什么?请选择吧!.\r\n#L0#改变发型(使用#b玩具城美发店高级会员卡#k)#l\r\n#L1#染色(使用#b玩具城染发高级会员卡#k)#l");						
		} else if (status == 1) {
			if (selection == 0) {
				beauty = 1;
				hairnew = Array();
				...
				cm.sendStyle("我可以改变你的发型,让它比现在看起来漂亮。你为什么不试着改变它下? 如果你有#b玩具城美发店高级会员卡#k,我将会帮你改变你的发型,那么选择一个你想要的新发型吧!", hairnew,5150007);
			} else if (selection == 1) {
				beauty = 2;
				haircolor = Array();
				...
				cm.sendStyle("我可以改变你的发色,让它比现在看起来漂亮. 你为什么不试着改变它下? 如果你有#b玩具城染发高级会员卡#k,我将会帮你改变你的发色,那么选择一个你想要的新发色吧!", haircolor,5151001);
			}
			}else if (status == 2){
			cm.dispose();
			if (beauty == 1){
