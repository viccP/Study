	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (em == null || !em.getProperty("entryPossible").equals("true")) {
				...
				cm.gainMeso(-1000000);
				em.getInstance("FoJ1").registerPlayer(cm.getChar());
			} else if (cm.getChar().getLevel() >= 31 && cm.getChar().getLevel() < 51) {
				cm.gainMeso(-1000000);
				em.getInstance("FoJ2").registerPlayer(cm.getChar());
			} else if (cm.getChar().getLevel() >= 51 && cm.getChar().getLevel() < 71) {
				cm.gainMeso(-1000000);
				em.getInstance("FoJ3").registerPlayer(cm.getChar());
			} else if (cm.getChar().getLevel() >= 71 && cm.getChar().getLevel() < 91) {
				cm.gainMeso(-1000000);
				em.getInstance("FoJ4").registerPlayer(cm.getChar());
			} else {
				cm.gainMeso(-1000000);
				em.getInstance("FoJ5").registerPlayer(cm.getChar());
			}
			cm.dispose();
		}
	}
