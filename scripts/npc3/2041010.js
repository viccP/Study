...
function action(mode, type, selection){
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("需要消耗#b#z5152007##k一张！。\r\n\r\n#L0##b进行整形手术#k(使用#b#z5152007##k)#l");
		} else if (status == 1) {
		if (selection == 0) {
				facenew = Array();
				...
				cm.sendStyle("我可以改变你的脸型,让它比现在看起来漂亮. 你为什么不试着改变它下? 如果你有#b射手村整形手术高级会员卡#k,我将会帮你改变你的脸型,那么选择一个你想要的新脸型吧!", facenew,5152007);
			}else if(selection == 1){
				beauty = 1;
				if (cm.getChar().getGender() == 0) {
				...
				cm.sendStyle("请选择你喜欢的颜色.", colors,5152007);
			}
		}else if (status == 2){			
			cm.dispose();
			if(beauty == 0){
