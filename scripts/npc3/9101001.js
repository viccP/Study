...
if (cm.getLevel() > 10 ) {  
    cm.sendSimple ("#b您好，我是冒险岛福利发放员。您要领取您今天的工资吗\r\n#d\r\n #L0#我要领取普通玩家工资#l   \r\n#L1#我要领取VIP1工资#l\r\n#L2#我要领取VIP2工资");
    } else ｛
    cm.sendOk("等不起，您的等级不1 0级，所以不能给你发放工资，如果你达到120以后，每天都可以在我这里领取\r\n100商城点卷\r\n100元宝\r\n100万游币")    }
}
...
cm.serverNotice("『福利公告』：【"+ cm.getChar().getName() +"】在自由市场【彼特】那里领取了1百万游戏币、100元宝、100点卷");
	cm.dispose();}
} else if (selection == 1) {//VIP1
           if (cm.getBossLog('HYGZ') >= 1 && cm.getPlayer().getvip() != 1) {
            cm.sendOk("抱歉，尊敬VIP玩家你今天己经领取工资，请明天再来找我吧！或者是你VIP不够");
	    cm.dispose();
        }else{
            cm.gainMeso(1000000);
            cm.setzb(100);
...
cm.serverNotice("『福利公告』：【"+ cm.getChar().getName() +"】在自由市场【彼特】那里领取了1百万游戏币、100元宝、100点卷");
	cm.dispose();}
       ...
       if (cm.getBossLog('HYGZ') >= 1&& cm.getPlayer().getvip() != 2) {
            cm.sendOk("抱歉，尊敬VIP玩家你今天己经领取工资，请明天再来找我吧！");
	    cm.dispose();
        }else{
            cm.gainMeso(1000000);
