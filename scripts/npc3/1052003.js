		...
		else
		{
			if (mats instanceof Array) {
				for(var i = 0; complete && i < mats.length; i++)
				{
					if (matQty[i] * qty == 1)	{
						...
						if (last_use && i == mats.length - 1)//last item
							iter = cm.getPlayer().getInventory(MapleInventoryType.USE).listById(mats[i]).iterator();
						else 
				...
				if (last_use == true)//only item is use
					iter = cm.getPlayer().getInventory(MapleInventoryType.USE).listById(mats).iterator();
