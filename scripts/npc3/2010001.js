...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		status++;
			...
			status--;
			if (status == 0) {
				cm.sendSimple("您好！我是美发店的院长#p2010001#！如果你有#b#t5150005##k或#b#t5151005##k，就放心把头发交给我处理吧。怎么样？你想做什么？请选择吧。\r\n#b#L0#换发型（用高级会员卡）#l\r\n#L1#染头发（用高级会员卡）#l");
			} else if (status == 1) {
				beauty = selection;
				if (selection == 0) {
					hairnew = Array();
					...
					cm.sendStyle("我们可以为你改变发型。你是不是已经厌倦了现在的发型？如果你有#b#t5150038##k，我们就可以给你换新的发型。我会让你满意的。怎么样？", hairnew, 5150038);
				}
			} else if (status == 2){
				if (beauty == 0){
					...
					if (cm.haveItem(5150038) == true){
						...
						cm.sendOk("嗯… 看来你没有#b#t5150038##k。如果你没有会员卡，我们不能为你修剪头发！");
