		...
		if (status == 0) {
			...
			if (!cm.isLeader()) { // not party leader
				...
				if (next) {
					...
					cm.sendOk("You either aren't in the same map with your partner, or you two are the same gender, or one of you is already married, or the leader of the party is the Bride. Remember, you have to be level 10+ to get married.");
					cm.dispose();
				}
			}
		}
			...
			if (cm.getPlayer().getMeso() >= 25000000) {
				...
				if (cm.getPlayer().getMarriageQuestLevel() == 50) {
					...
					if (cm.haveItem(4032056, 1)) {
						cm.sendNext("Great, Heres my permission.");
						cm.removeAll(4032056);
				...
				//	cm.createMarriage(partyi.get(0), partyi.get(1));//rings[1]);
					...
					if (cm.getPlayer().getGender() == 0) {
						...
						cm.getPlayer().saveToDB(true, true);
                                ...
                                if (cm.getPlayer().getPartnerId() < 1) {
                                var StringLine2 = cm.getPlayer().getName() + " is now married! Congratulate him/her!";
                                cm.worldMessage(StringLine2);
