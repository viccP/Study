...
if (cm.haveItem(4031120) && cm.getChar().getMapId() == 229020000){
...
if (selection == 0) {
	if(cm.getzb() >= 5000) {
            cm.setzb(-5000);
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友开始挑战绯红骑士团【玛玛哈】！"); 
        cm.summonMob(9001014, 10000000, 20000, 1);玛玛哈 
        }else{
        cm.sendOk("抱歉你没有5000元宝。我不能为您召唤"); 
	cm.dispose();}
} else if (selection == 1) {
	if(cm.haveItem(4021010)) {
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友成功的击败绯红骑士团【玛玛哈】开始挑战第五层");
        cm.warp(674030200, 0);
