...
function action(mode, type, selection) {
   ...
   status--;
	...
	text += "挑战了副本是不是获得了#v4001008#呀！这个道具是无法保留的！#b再次进入会清空处理。请慎重！#k"
	text += "\r\n#L2##b#v4001008#兑换#v2022615# 1张换1个 <几率开黑色刺飞镖>#k\r\n"; 
	text += "\r\n#L1##r#v4001008#兑换#v2022615# 5张换5个 <几率开黑色刺飞镖>#k";
	text += "\r\n#L3##d#v4001008#兑换#v2022615# 10张换10个 <几率开黑色刺飞镖>#k";
        text += "\r\n#L4##b获取精灵吊坠<30天使用权>-102000点卷#k";
        text += "\r\n#L5##b获取温暖的围脖<全属性+2.30天使用权>-202000点卷#k";
        cm.sendSimple(text);
    } else if (status == 1) {
           if (selection == 0) {      
	   cm.warp(910000000); 
	  	...
	  	if (cm.haveItem(4001008, 5)) { 
		   	cm.gainItem(4001008, -5);
            		cm.gainItem(2022615, 5);
			cm.sendOk("换购成功！");
                  	cm.dispose();
                   	} else {
	           	cm.sendOk("看来你没有需要的道具啊..这需要你完成我旁边的NPC所提供的任务才能获得这个物品！");
			cm.dispose(); }
    }else if  (selection == 3) {
	if (cm.haveItem(4001008, 10)) { 
		   	cm.gainItem(4001008, -10);
            		cm.gainItem(2022615, 10);
			cm.sendOk("换购成功！");
                  	cm.dispose();
                   	} else {
	           	cm.sendOk("看来你没有需要的道具啊..这需要你完成我旁边的NPC所提供的任务才能获得这个物品！");
...
cm.getChar().saveToDB(true,true);
  cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.cherry.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"〖双倍经验〗" + " : " + " [" + cm.getPlayer().getName() + "]换取了精灵吊坠[30天使用权]！！！！",true).getBytes()); 
		   cm.dispose();
	   }else{
