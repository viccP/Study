...
if ((cm.getzb() >= 10000) && cm.getChar().getMapId() == 926100400){
    cm.sendSimple ("#b您是蘑菇公主派来救王子的吗。暴力熊就隐藏在这里，平时是看不到他的,如果您有1万元宝，我可以使用魔法让他显示。\r\n#d剩余:#r" + cm.getzb() + "元宝 【需要10000元宝】\r\n\r\n#L0#挑战暴力熊#l      #L64#回自由市场#l ");
    } else {
    cm.sendOk("启动我的能力【需要10000元宝】。或你所在的地图不正确\r\n#d剩余:#r" + cm.getzb() + "元宝.")    }
}
...
if (selection == 0) {
	if(cm.getzb() >= 10000) {
            cm.setzb(-10000);
cm.serverNotice("『暴力熊公告』：【"+ cm.getChar().getName() +"】带领他的队友开始挑战A级BOSS【暴力熊】！");
        cm.summonMob(9420542, 60000000, 200000, 1);暴力熊
	cm.dispose();
        }else{
        cm.sendOk("你没有足够的元宝，请即时充值。"); 
	cm.dispose();}
} else if (selection == 1) {
	if(cm.haveItem(4021010)) {
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友成功的击败绯红骑士团【吉米】开始挑战第六层");
        cm.warp(803200000, 0);
