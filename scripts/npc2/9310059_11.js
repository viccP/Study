	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (cm.getParty() == null) { // 不是组队
				...
				if (next) { //加载活动脚本
					var em = cm.getEventManager("cangbaokn");
                                        cm.gainItem(5252001, -1);
					cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.cherry.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"[藏宝活动]" + " : " + " [" + cm.getPlayer().getName() + "]进入了副本地图.在" + cm.getC().getChannel() + "频道。模式为[困难]",true).getBytes()); 
                                         //cm.getChar().setsg2(1);
					if (em == null) {
