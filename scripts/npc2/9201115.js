...
if (cm.haveItem(4031120) && cm.getChar().getMapId() == 803200000){
...
if (selection == 0) {
	if(cm.getzb() >= 30000) {
            cm.setzb(-30000);
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友开始挑战绯红骑士团【暗影杀手】！"); 
        cm.summonMob(9400593, 500000000, 4000000, 1);暗影杀手    
        }else{
        cm.sendOk("抱歉你没有30000元宝。我不能为您召唤"); 
	cm.dispose();}
} else if (selection == 1) {
	if(cm.haveItem(4021010)) {
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友成功的击败绯红骑士团【暗影杀手】开始挑战第七层");
        cm.warp(970041809, 0);
	...
	cm.gainItem(4001035, -100);
        ...
        cm.sendOk("不好意思你需要#v4001035#1个通关凭证"); 
	cm.dispose(); } 
} else if (selection == 3) { 
