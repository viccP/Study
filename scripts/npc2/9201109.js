...
if (cm.haveItem(4031120) && cm.getChar().getMapId() == 970040503){
...
if (selection == 0) {
	if(cm.getzb() >= 10000) {
            cm.setzb(-10000);
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友开始挑战绯红骑士团【幽灵船长】！"); 
        cm.summonMob(9420513, 10000000, 100000, 1);幽灵船长
        }else{
        cm.sendOk("抱歉你没有10000元宝。我不能为您召唤"); 
	cm.dispose();}
} else if (selection == 1) {
	if(cm.haveItem(4021010)) {
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友成功的击败绯红骑士团【幽灵船长】开始挑战第九层");
        cm.warp(803001200, 0);
