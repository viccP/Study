...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			if (cm.getQuestStatus(100010) == net.sf.odinms.client.MapleQuestStatus.Status.COMPLETED) {
				cm.sendOk("You're truly a hero!");
				cm.dispose();
			} else if (cm.getQuestStatus(100009).getId() >= net.sf.odinms.client.MapleQuestStatus.Status.STARTED.getId()) {
				cm.completeQuest(100009);
				if (cm.getQuestStatus(100009) == net.sf.odinms.client.MapleQuestStatus.Status.COMPLETED) {
