...
function action(mode, type, selection) {
    ...
    cm.teachSkill(5221010,0,5);
            ...
            if (cm.c.getPlayer().getInt() >= 40000) {
            ...
            if(cm.getJob() == 1000 && cm.getLevel()>=1000){
		...
		cm.teachSkill(14111005,0,20);//三连环极光破
                cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer() + "] 在NPC：可乐小姐 最终转职成功！");
            ...
            if(cm.getJob() == 2000 && cm.getLevel() >=10){
                ...
                //cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer() + "] 在NPC：可乐小姐 快速转职为战神职业！");
                cm.sendOk("转职成功！加油锻炼，当你变的强大的时候记的来找我哦！");
            } else if(cm.getJob() == 2111 && cm.getLevel() >=1000){
                cm.getPlayer().gainAp(5);
