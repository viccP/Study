...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			if (cm.haveItem(4280001)&&cm.getNX() >= 1000) {
				cm.sendYesNo("是否消耗1000点使用#b重生迷之蛋#k？",2);
			} else {
				cm.sendOk("开打重生迷之蛋需要消耗1000点卷。",2);
				cm.dispose();
			}
		} else if (status == 1){	
			...
			if(finalitem.length != 0){
				...
				if(Laba == 1){
					cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.cherry.tools.MaplePacketCreator.getItemMega(cm.getC().getChannel(),cm.getPlayer().getName() + " : " + "从重生迷之蛋里获得！大家祝贺它吧！",toDrop, true).getBytes());
				}
