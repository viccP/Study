			...
			// 如果是在一个组队上.没有导言.直接检查组队条件
				...
				if (next) { //加载活动脚本
					...
					if (em == null) {
						...
						cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.cherry.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"[组队任务 - 天空之城]" + " : " + " [" + cm.getPlayer().getName() + "]与小组开始了女神挑战。在" + cm.getC().getChannel() + "频道",true).getBytes()); 
						if (cm.getLevel() > 1 ) {
