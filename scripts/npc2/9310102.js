	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (cm.getParty() == null) { // 不是组队
				cm.sendOk("勇士..我在神木村观察了很久..感受到了#b黑暗龙王#k的恐怖气息.我已经找到了#b神秘的巢穴#k.它就潜伏在里面!\r\n#b你是否愿意创建个组队一探究竟?#d\r\n完成后可以获得#b黑龙箱子#k\r\n#r双击可打开,可以获得技能册,高级装备,点卷道具,卷轴");			//cm.mapMessage("请通过组队进入副本！需要30~60级。玩家2~5名！在这个组队挑战任务中，你总共完成了 "+cm.getboss()+" 次！");		
				cm.dispose();
				...
				if (next) { //加载活动脚本
					...
					cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.cherry.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"[枫叶之旅]" + " : " + " [" + cm.getPlayer().getName() + "]进入了副本地图.在" + cm.getC().getChannel() + "频道",true).getBytes()); 
                                         cm.getChar().setsg2(1);
					if (em == null) {
