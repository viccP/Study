...
var text = "跳到上面去,然后收集1个音乐碟子.交给我即可完成本关.\r\n#L1#进入下#l\r\n\r\n现在服务器时间为:" + cm.getHour() + "时:" + cm.getMin() + "分:" + cm.getSec() + "秒";
cm.sendSimple(text);
...
cm.sendOk("#v4001056# 需要1个, 目前数量不足!");
cm.dispose();
}
}else if (selection == 2) {
if (cm.getHour() < 0) {
...
if (cm.itemQuantity(4001054) < 50) {
    cm.sendOk("请找到#v4001054# 消灭上面的花即可爆出 需要50个！怪物已经刷出来了！");
	
    cm.dispose();
       } else if (cm.itemQuantity(4001054) > 49 && cm.isLeader()) {
                   // cm.gainItem(4001049,-1); 
                    cm.gainItem(4001054,-50);  
cm.summonMob(9300039, 1500000, 450000, 1);//船长5E血

cm.serverNotice("【女神组队任务】:[" + cm.getPlayer() + "] 成功召唤出了远古精灵,他们能否打败远古精灵 救出女神呢?");             
...
cm.sendOk("您不能选择这个奖励关卡.请消灭远古精灵,救出女神!!!");
cm.dispose();
}else { 
