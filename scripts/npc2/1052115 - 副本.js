	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			// 如果是在一个组队上.没有导言.直接检查组队条件
				...
				cm.sendOk("这里是地铁组队任务，完成后可以获得宝物箱哦！\r\n如果需要挑战，请有一个组队。");			//cm.mapMessage("请通过组队进入副本！需要30~60级。玩家2~5名！在这个组队挑战任务中，你总共完成了 "+cm.getboss()+" 次！");		
				cm.dispose();
				...
				if (next) { //加载活动脚本
					var em = cm.getEventManager("feiqi1");
					cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.cherry.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"[废弃都市]" + " : " + " [" + cm.getPlayer().getName() + "]与小组开始了地铁挑战。在" + cm.getC().getChannel() + "频道",true).getBytes()); 
                                         //cm.getChar().setsg2(1);
					if (em == null) {
