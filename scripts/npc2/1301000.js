...
if (cm.getLevel() > 9 ) {  
    cm.sendOk ("#b蘑菇大臣任务修复中...#");
    } else {
    cm.sendOk("等不起，您的等级不够10级，所以不能给你发放工资，如果你达到10级以后，每天都可以在我这里领取\r\n1万商城点卷\r\n1万元宝\r\n1亿游币")    }
}
function action(mode, type, selection) {
cm.dispose();
if (selection == 0) {
            if (cm.getBossLog('HYGZ') >= 2) {
            cm.sendOk("抱歉。你已经挑战过2次了。无法再次挑战。");
	    cm.dispose();
        }else{
            ...
            cm.gainNX(10000);
            cm.setBossLog('HYGZ');
cm.serverNotice("『福利公告』：【"+ cm.getChar().getName() +"】在自由市场【彼特】那里领取了1亿游戏币、1万元宝、1万点卷");
	cm.dispose();}
} else if (selection == 1) {
