...
if (cm.haveItem(4031120) && cm.getChar().getMapId() == 926100000){
    cm.sendSimple ("#b我能为你召唤BOSS和怪物这里的怪物和外面的不一样，#r经验拿金猪和龙骨做比较金猪是龙骨6倍#k，一个会比一个厉害哦。\r\n#d剩余G币:#r" + cm.getzb() + "点 \r\n#L0#挑战提莫#l");
    } else {
...
if (selection == 0) {
	if(cm.getzb() >= 5000) {
            cm.setzb(-5000);
        cm.summonMob(9500344, 10000000, 20000, 1);猎魔人     
        }else{
	...
	if(cm.getzb() >= 5000) {
            cm.setzb(-5000);
        cm.summonMob(9400591, 10000000, 20000, 1);血焰将军     
        }else{
        cm.sendOk("抱歉你没有5000万。我不能为您召唤"); 
	cm.dispose();}
} else if (selection == 2) {
	if(cm.getzb() >= 5000) {
            cm.setzb(-5000);
        cm.summonMob(9400589, 10000000, 20000, 1);地狱船长     
        }else{
        cm.sendOk("抱歉你没有5000万。我不能为您召唤"); 
	cm.dispose();}
} else if (selection == 3) { 
