...
function action(mode, type, selection) {
    ...
    if (cm.getChar().getMap().getId() == 220000000) {
        if (status == 0) {
            cm.sendSimple("你好, 我的名字是Spieglemann, 我主持怪物嘉年華活動.I am trying to find players who are willing to fight for there lives against each other in a fair competition, and that is why I host the Monster Carnival PQ.\r\n\r\n#L0#我想參加#l\r\n#L1#怪物嘉年華是什麼?#l\r\n#L2#Detailed Information#l\r\n#L3#Trade Maple Coins for Equipment#l");
        } else if (status == 1) {
            if (selection == 0) {
                if (cm.getLevel() > 30 && cm.getLevel() < 250) {
                    cm.getChar().saveLocation(SavedLocationType.MONSTER_CARNIVAL);
                    cm.warp(980000000, 0);
                    cm.dispose();
                    return;
                } else {
                    cm.sendOk("非常抱歉，只有30~50級之前才可以參加怪物嘉年華.\r\n 參加怪物嘉年華.");
                    cm.dispose();
                    return;
                }
            } else if (selection == 1) {
        ...
        if (status == 0) {
            if (cm.getChar().getParty() != null) {
                var shi = "恭喜你勝利了!!! 這是一個令人驚訝的表現! 我很期望下次還能見到同樣的精彩!\r\n\r\n怪物嘉年華排行:";
        ...
        if (status == 0) {
          if (cm.getChar().getParty() != null) {
            var shiu = "很可惜失敗了... 但是, 你表現的相當精彩! 你的團隊合作與戰鬥方式令人驚訝! 我很期望下次還能見到同樣的精彩!\r\n\r\n怪物嘉年華排行: ";
            switch (cm.calculateCPQRanking()) {
              case 10:
                shiu += "A";
                cm.sendOk(shiu);
