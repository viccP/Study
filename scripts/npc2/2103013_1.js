	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			// 如果是在一个组队上.没有导言.直接检查组队条件
				...
				if (next) { //加载活动脚本
					var em = cm.getEventManager("wanmotapt");
					cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.cherry.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"[※万魔塔☆普通模式※]" + " : " + " [" + cm.getPlayer().getName() + "]带着队伍进入了※万魔塔☆普通模式※。在" + cm.getC().getChannel() + "频道",true).getBytes()); 
