...
if (cm.haveItem(4031120) && cm.getChar().getMapId() == 970041809){
...
if (selection == 0) {
	if(cm.getzb() >= 10000) {
            cm.setzb(-10000);
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友开始挑战绯红骑士团【大海兽】！"); 
        cm.summonMob(9500361, 10000000, 200000, 1);大海兽     
        }else{
        cm.sendOk("抱歉你没有10000元宝。我不能为您召唤"); 
	cm.dispose();}
} else if (selection == 1) {
	if(cm.haveItem(4021010)) {
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友成功的击败绯红骑士团【大海兽】开始挑战第八层");
        cm.warp(970040503, 0);
