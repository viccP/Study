	...
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
                                          ...
                                          if (selection == 0){
                                          cm.sendNext(""+name+"是女皇陛下最珍惜的部队，他是运用风的属性给敌人造成伤害。等级上限#b"+level+"#k级。\r\n例如技能#b灵魂冲刺#k，他将神圣之力负载刀剑上，给黑暗系的敌人给予非常大的伤害！")
}else if (selection == 1){
status = 2;
cm.sendSimple("你确定好了吗？成为了#p1104100#"+name1+"的部下！？#b\r\n#L0#是的！我无怨无悔成为您的部下！\r\n#L1#不要！让我再想想。。")
}
...
cm.MissionMake(cm.getPlayer().getId(),1001,0,0,0,0)//因为判断职业的函数不能用，所有就用高级任务来判断：）
cm.changeJob(1100)
}else if (selection == 1){
