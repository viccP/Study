...
var itemList = 
Array(     
			Array(2340000,900,1,1), //祝福
			Array(2000005,850,50,1), //超级药水
			Array(2049100,900,1,1) //混沌
);
...
function action(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 0) {
            cm.sendOk("天空副本通关抽奖：#v2049100#混沌卷轴/#v2340000#祝福卷轴/#v2000005#超级药水x50.随机抽取其一.");
            cm.dispose();
        }
        status--;
    }
    if (status == 0) {
        if (cm.haveItem(4170006, 1)) {
            cm.sendYesNo("天空副本通关抽奖：#v2049100#混沌卷轴/#v2340000#祝福卷轴/#v2000005#超级药水x50.随机抽取其一.");
        } else {
            cm.sendOk("天空副本通关抽奖：#v2049100#混沌卷轴/#v2340000#祝福卷轴/#v2000005#超级药水x50.随机抽取其一..你背包里有1个#b#t4170005##k吗?");
            cm.safeDispose();
        }
    } else if (status == 1) {
